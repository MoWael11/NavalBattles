  ...
  ships.forEach(function (ship) {
    do {
      if (Math.floor(Math.random() * 2) === 1) {
        ...
        ship.height = temp;
        ship.shipImg.src = "images/shape-rotated-id-" + ship.id + ".png";
        ship.rotation = 90;
      } else {
        ship.shipImg.src = "images/shape-id-" + ship.id + ".png";
        ship.rotation = 0;
      }
      do {
...
function playerTurn(event) {
  ...
  if (computerMap.cheksAllShipShotted()) {
    winner.text = "You won congratulations!";
    gameStarted = false;
    exitingMassege = true;
    gameEnded = true;
    return false;
...
class Ship {
  constructor({ position, width, height, id, size }) {
    ...
    this.shipImg.src = "images/shape-id-" + this.id + ".png";
  }
  draw() {
  ...
  updateImage() {
    if (this.rotation === 0) {
      ...
      this.shipImg.src = "images/shape-rotated-id-" + this.id + ".png";
    }
  }
}
